...
public class Project_Gene_1 implements Serializable
{
	...
	{
		int sheetIndex = 0;
		XSSFSheet xsheet1 =null;
	
		try
		{
		XSSFWorkbook worksheet2 = new XSSFWorkbook(new FileInputStream( new File("GeneS2.xlsx")));
		
		xsheet1= worksheet2.getSheetAt(sheetIndex);
		
		}
		catch(Exception e){ e.printStackTrace();}

		int rows=0; 
		for (Row tempRow : xsheet1)
		{
...
double getActualSum()
{
 ...
 return sum;
  ...
  static double doAverageErr()
  {
   ...
   for(double d :Project_Gene_1.temperrList)
   {
	   count++;
	   sum+=d;
   }
  ...
  // System.out.println("Count ="+count);
   ...
   double avg =(double)sum/Project_Gene_1.temperrList.size();
   
   Project_Gene_1.errList.add(avg); 
  ...
  void writing_file() 
  {
	...
	//	{
		Project_Gene_1 qt  = new Project_Gene_1();
